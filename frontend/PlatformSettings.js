...
class PlatformSettings extends React.Component {
  ...
  onChange(key, value, max) {
    ...
    this.props.onChange(Object.assign({x, y, angle}, {[key]: Math.min(parseInt(max), parseInt(value))}))
  }
  render() {
    ...
    return (
      <div style={style}>
        ...
        <Slider style={sliderStyle} data-key={"angle"}value={this.props.angle} min={0} max={355} step={5}
          onChange={(v) => this.onChange('angle', v, 355)}
        />
      </div>
    );
  }
