...
class PlatformSettings extends React.Component {
  ...
  onChange(key, value, max) {
    ...
    this.props.onChange(Object.assign({ x, y, width, height, angle }, { [key]: Math.min(parseInt(max), parseInt(value)) }), key)
  }
  render() {
    ...
    return (
      <div style={style}>
        ...
        <label style={{ ...labelStyle, margin: '0 1vw' }}>Angle: {this.props.angle}&#730;</label>
        <Slider style={sliderStyle} value={this.props.angle} min={0} max={175} step={5} onChange={(v) => this.onChange('angle', v, 175)} />
      </div>
    );
  }
}
